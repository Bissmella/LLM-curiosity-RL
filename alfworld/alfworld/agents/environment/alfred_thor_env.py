...
class AlfredThorEnv(object):
    ...
    def reset(self, json_file_id=None,env_id=None):
        # set tasks
        batch_size = self.batch_size
        # reset envs

        if self.train_eval == "train":
            if json_file_id is None:
                tasks = random.sample(self.json_file_list, k=batch_size)
            else:
                tasks = self.json_file_list[json_file_id : json_file_id + batch_size]
        else:
            if len(self.json_file_list) - batch_size > batch_size:
                tasks = [
                    self.json_file_list.pop(random.randrange(len(self.json_file_list)))
        ...
        if env_id==None:
            ...
            self.action_queues[env_id].put((None, True, tasks[env_id]))
