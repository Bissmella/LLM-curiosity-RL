...
class AlfredExpert(textworld.core.Wrapper):
    def __init__(self, env=None, expert_type=AlfredExpertType.HANDCODED):
        ...
        if expert_type not in (
            AlfredExpertType.HANDCODED,
            AlfredExpertType.PLANNER,
            AlfredExpertType.NONE,
        ):
            msg = "Unknown type of AlfredExpert: {}.\nExpecting either '{}' or '{}'."
            msg = msg.format(
                expert_type, AlfredExpertType.HANDCODED, AlfredExpertType.PLANNER
            )
    ...
    def _gather_infos(self):
        ...
        elif self.expert_type == AlfredExpertType.NONE:
            pass
        else:
            raise NotImplementedError(
                "Unknown type of AlfredExpert: {}.".format(self.expert_type)
            )
    ...
    def load(self, gamefile):
        ...
        self.request_infos.facts = self.request_infos.facts or (
            self.expert_type == AlfredExpertType.HANDCODED
        )
...
class AlfredTWEnv(object):
    ...
    def __init__(self, config, train_eval="train"):
        ...
        if config["env"]["goal_desc_human_anns_prob"] > 0:
            msg = (
                "Warning! Changing `goal_desc_human_anns_prob` should be done with"
    ...
    def collect_game_files(self, verbose=False):
        ...
        elif self.train_eval == "eval_train_unseen_text":
            data_path = os.path.expandvars(self.config["dataset"]["eval_train_unseen_text"])
        elif self.train_eval == "valid_train":
            data_path = os.path.expandvars(self.config["dataset"]["valid_train"])
        ...
        print(
            f"Overall we have {len(self.game_files)} games in split={self.train_eval}"
        )
    ...
    def get_game_logic(self):
        self.game_logic = {
            "pddl_domain": open(
                os.path.expandvars(self.config["logic"]["domain"])
            ).read(),
        ...
        try:
            ...
            while not done:
                ...
                if random_perturb:
                    if (
                        steps <= random_start
                        or random.random() < random_prob_after_state
                    ):
