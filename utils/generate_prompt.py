...
def Glam_prompt(info, past_transitions):
    prompt = ""
    prompt += "Possible actions of the agent: {}\n".format(
        ", ".join(info["possible_actions"])
    )
...
def Glam_prompt_test(info, past_transitions):
    prompt = ""
    #prompt += "Possible actions of the agent: {}\n".format(
    #    ", ".join(info["possible_actions"])
    #)
    ...
    prompt += "Next action of the agent: "
    return prompt
def Glam_prompt_test2(info, past_transitions):
    prompt = ""
    prompt += "Goal: {}\n".format(info["goal"])
    #if "context" in info.keys():
    #    prompt += "Context: {}\n".format(", ".join(info["context"]))
...
def paraphrase_prompt(info):
    ...
    return prompt
def prompt_maker(dico,element=["obs"],nb=0,limite=None):
    prompt = ""
    if "possible_actions" in element:
        prompt += "Possible actions of the agent: {}\n".format(", ".join(dico['info']["possible_actions"]))
    prompt += "Goal: {}\n".format(dico['info']["goal"])
    if "obs" in element:
        prompt += "Current Observation: {}\n".format(", ".join(dico['info']["obs"]))
    else:
        
        prompt += "Current Observation: <no obs>\n"
    for d in dico["transition_buffer"][(-1-nb):limite]:
        if "past obs" in element:
            prompt += "Past Observation: {}\n".format(", ".join(d["obs"]))
        if "past action" in element:
