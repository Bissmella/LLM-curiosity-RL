...
from .utils import load_hf_vlm_and_tokenizer_idefics as load_hf_vlm_and_tokenizer
...
class HF_LLM(BaseLLM):
    ...
    def __get_past_key_values(self, input):
        ...
        result = self._VLM_model(**input_batch)["past_key_values"]
        tensor_results = []
    ...
    def generate(self, contexts,prompts=["<DETAILED_CAPTION>"],sample=False, return_logprobs=False,temperature=1, **kwargs):
        ...
        with torch.no_grad():
            for _ii in range(len(contexts)):
                ...
                # print(len(contexts),generated_ids.keys())
    ...
    def forward(
        ...
        **kwargs,
        ...
        with torch.no_grad() if not require_grad else nullcontext():
            ...
            #print("you should not pass",tokenized_contexts)
            ...
            #print("self.__input_encoder",self.__input_encoder)
            if self.__input_encoder is not None :
                ...
                for step in range(len(contexts) // _minibatch_size + 1):
                    ...
                    for _i in range(
                        ...
                        #_current_context["labels"]=[_current_labels["input_ids"]]
            ...
            for step in range(len(batch_inputs) // _minibatch_size + 1):
                ...
                #print("_inputs_representation",_inputs_representation)
                ...
                minibatch = {}
                #print("minibatch_inputs",[(key,minibatch_inputs[key]) for key in minibatch_inputs.keys()])
                ...
                #print("minibatch",minibatch.keys(),minibatch["past_key_values"].shape)
