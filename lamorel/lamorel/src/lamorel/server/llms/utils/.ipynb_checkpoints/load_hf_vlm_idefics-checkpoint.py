...
from transformers import AutoProcessor, AutoModelForCausalLM , AutoModelForSeq2SeqLM, AutoTokenizer, AutoConfig
from transformers import Idefics2Processor, Idefics2ForConditionalGeneration
class ModelTypesEnum(Enum):
    causal = Idefics2ForConditionalGeneration
    seq2seq = AutoModelForSeq2SeqLM
    


def load_hf_vlm_and_tokenizer_idefics(type, path, pretrained):
    print("Loading vlm model {}".format(path))
    #tokenizer = AutoTokenizer.from_pretrained(path, trust_remote_code=True)
    processor = Idefics2Processor.from_pretrained(path, trust_remote_code=True)
