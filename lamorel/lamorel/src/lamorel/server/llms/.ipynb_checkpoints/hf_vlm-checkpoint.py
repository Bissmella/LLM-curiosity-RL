...
from .utils import load_hf_vlm_and_tokenizer
...
class HF_LLM(BaseLLM):
    ...
    def __init__(self, args, devices, use_cpu):
        ...
        self._LLM_tokenizer, _model_constructor, num_layers = (
            load_hf_model_and_tokenizer(
                args.model_type, args.model_path, args.pretrained
            )
        )
        use_vlm=args.use_vllm
        constructor_kwargs = {"trust_remote_code": True}
        if use_vlm:
            self._VLM_processor, _vlm_constructor = load_hf_vlm_and_tokenizer(args.vlm_model_type, args.vlm_model_path, args.pretrained)
        ...
        if use_cpu:
            ...
            with init_empty_weights():
                ...
                if use_vlm:
                    self._VLM_model = _vlm_constructor(**constructor_kwargs)
                    self._VLM_model2 = _vlm_constructor(**constructor_kwargs)
                    self._VLM_model.tie_weights()
                    self._VLM_model2.tie_weights()
                    device_map = infer_auto_device_map( model=self._VLM_model,max_memory={_device: torch.cuda.mem_get_info(f"cuda:{_device}")[0] for _device in devices},)
                constructor_kwargs["device_map"] = device_map
            if use_vlm:
                
                self._VLM_model = _vlm_constructor(**constructor_kwargs)
                self._VLM_model2 = _vlm_constructor(**constructor_kwargs)
                for param in self._VLM_model.parameters():
                    param.requires_grad = True
                for param in self._VLM_model2.parameters():
                    param.requires_grad = True
            ...
            if use_vlm:
                ...
                for param in self._LLM_model.parameters():
                    param.requires_grad = False
            print(self._VLM_model2)
        ...
        if args.model_type == "causal":
            ...
            if args.pre_encode_inputs:
                self.__input_encoder = (
                    lambda input: self.__get_input_embedding_from_encoder(input)
                )
        ...
        self.__synchronize_gpus_after_scoring = (
            args.parallelism.synchronize_gpus_after_scoring
        )
        self.__empty_cuda_cache_after_scoring = (
            args.parallelism.empty_cuda_cache_after_scoring
        )
    ...
    def __pad_sequence(self, sequence, size, right=True):
        ...
        if right:
            ids = sequence["input_ids"] + [
                self.pad_token for _ in range(size - sequence_size)
            ]
            mask = sequence["attention_mask"] + [0 for _ in range(size - sequence_size)]
        else:
            
            ids = [self.pad_token for _ in range(size - sequence_size)] + sequence[
                "input_ids"
            ]
    ...
    def __concat_sequences(self, sequence_a, sequence_b, sequence_size=None):
        if sequence_size is None:
            sequence_size = len(sequence_a["input_ids"]) + len(sequence_b["input_ids"])
        return self.__pad_sequence(
            {
                "input_ids": sequence_a["input_ids"] + sequence_b["input_ids"],
                "attention_mask": sequence_a["attention_mask"]
                + sequence_b["attention_mask"],
            },
            sequence_size,
        )
    ...
    def __build_decoder_minibatch(self, outputs, inputs, inputs_representation=None):
        ...
        batch = {"input_ids": [], "attention_mask": [],"pixel_values":[],"decoder_input_ids":[]}

        output_max_size = max([len(o["input_ids"]) for o in outputs])
        for idx, output in enumerate(outputs):
            _output = self.__pad_sequence(output, output_max_size)
            if inputs_representation is None:
                
                batch["input_ids"].append(
                    inputs[idx]["input_ids"][0] + _output["input_ids"]
                )
            else:
                batch["input_ids"].append(_output["input_ids"])

            batch["attention_mask"].append(
                inputs[idx]["attention_mask"][0] + _output["attention_mask"]
            )
            batch["pixel_values"].append(
                inputs[idx]["pixel_values"][0] 
            )
    ...
    def __build_encoder_decoder_minibatch(
        self, outputs, inputs=None, inputs_representation=None
    ):
        """
        Set state as encoder's input and action as decoder's input
        """
        assert inputs is not None or inputs_representation is not None
        batch = {"decoder_input_ids": [], "decoder_attention_mask": []}
    ...
    def __get_input_embedding_from_encoder(self, input):
        ...
        return self._LLM_model.encoder(**input_batch, return_dict=False)[0].to(
            self.device
        )
    ...
    def __get_past_key_values(self, input):
        input_batch = {}
        for key, value in input.items():
            if key =="input_ids" or key =="decoder_input_ids":
                input_batch[key] = torch.tensor(value[0], device=self.device).long()
            else:
                input_batch[key] = torch.tensor(value[0], device=self.device).float()
        #input_batch["use_cache"]=True
        result = self._VLM_model(**input_batch)
        
        return result["encoder_last_hidden_state"]
        tensor_results = []
        for _layer in result:
            
            layer_tensor = torch.stack([_l.to(self.device) for _l in _layer[2:]], dim=1)
            tensor_results.append(layer_tensor)

        return torch.stack(tensor_results, dim=1)

    def generate(self, contexts,prompts=["<DETAILED_CAPTION>"],sample=False, return_logprobs=False,temperature=1, **kwargs):
        generations = []
        
        with torch.no_grad():
            for _ii in range(len(contexts)):
                frames=contexts[_ii]
                prompt = [prompts[_ii]]
                ...
                generated_ids = self._VLM_model2.generate(
                ...
                parsed_answer = [
                    self._VLM_processor.post_process_generation(
                        gent, task=prompt[0], image_size=(300, 300)
                    )
                    for gent in generated_text
                ]

                generations.append(
                    {"text": parsed_answer[-1][prompt[0]]}
                )
    ...
    def forward(
        self,
        module_function_keys,
        contexts,# contexts here is a list of tuple(image; prompt)
        candidates=None,
        require_grad=False,
        minibatch_size=None,
        **kwargs,
        ...
        with torch.no_grad() if not require_grad else nullcontext():
            ...
            tokenized_contexts = [
                ...
                self._VLM_processor(
                    text=context[0], images=context[1],return_tensors=None
                )
            ...
            for _w, _candidates in enumerate(candidates):
                _ids_tables[_w] = [
                    i
                    for i in range(
                        len(batch_inputs), len(batch_inputs) + len(_candidates)
                    )
                ]
                ...
                outputs = [
                    self._VLM_processor.tokenizer(
                        output, add_special_tokens=False, padding=True, return_tensors=None
                ...
                if self.model_type == "causal":
                    ...
                    cut_input = tokenized_contexts[_w]
                    
                    padded_input = self.__pad_sequence(
                        cut_input, contexts_max_size - 1, right=False
                    )
                    
                    end_of_input = {
                        _k: [_v[0][-1]] for _k, _v in tokenized_contexts[_w].items()
                    }
                    ...
                    padded_input = self.__pad_sequence(
                        tokenized_contexts[_w], contexts_max_size
                    )
            ...
            _minibatch_size = (
                ...
                else self._scoring_minibatch_size
            )
            ...
            if self.__input_encoder is not None :
                ...
                for step in range(len(contexts) // _minibatch_size + 1):
                    _input = {}
                    for _i in range(
                        step * _minibatch_size,
                        min((step + 1) * _minibatch_size, len(contexts)),
                    ):
                        if len(_ids_tables[_i]) == 0:
                            break
                        _current_context = batch_inputs[_ids_tables[_i][0]]
                        _current_labels = batch_outputs[_ids_tables[_i][0]]
                        _current_context["decoder_input_ids"]=[_current_labels["input_ids"]]
                    ...
                    if len(_input.keys()) > 0:
                        ...
                        _current_candidates = candidates[
                            step * _minibatch_size : (step + 1) * _minibatch_size
                        ]
                        batch_input_representations.append(
                            torch.repeat_interleave(
                                result,
                                torch.tensor(
                                    [len(_c) for _c in _current_candidates],
                                    device=self.device,
                                ),
                                dim=0,
                            )
            ...
            for step in range(len(batch_inputs) // _minibatch_size + 1):
                ...
                current_minibatch_size = min(
                    _minibatch_size, len(batch_inputs) - step_idx
                )
                if current_minibatch_size <= 0:
                    break
                #print("batch_input_representations",batch_input_representations)
                _inputs_representation = (
                    batch_input_representations[
                        step_idx : step_idx + current_minibatch_size
                    ]
                    if batch_input_representations is not None
                    else None
                )
                ...
                for key, value in minibatch_inputs.items():
                  if key in ["decoder_input_ids",'encoder_outputs']:
                    if key =="output_hidden_states":
                        pass
                    elif key in ["encoder_outputs", "past_key_values",'encoder_last_hidden_state']:
                        minibatch[key] = value
                    else:
                ...
                _outputs = self._VLM_model(**minibatch)  # Get scores before softmax
                lamorel_logger.debug(
                ...
                for _key in module_function_keys:
                    ...
                    if _key == "__score":
                        results = _fn(
                            _outputs,
                            minibatch,
                            batch_inputs[step_idx : step_idx + current_minibatch_size],
                        )
                    else:
                        results = _fn(
                            _outputs,
        ...
        if self.__synchronize_gpus_after_scoring:
            lamorel_logger.debug(
                f"Synchronizing GPUs on process {self.accelerator.process_index}"
            )
            for device in self.devices:
                torch.cuda.synchronize(device)
        if self.__empty_cuda_cache_after_scoring:
            lamorel_logger.debug(
                f"Emptying CUDA cache on process {self.accelerator.process_index}"
            )
            torch.cuda.empty_cache()

        lamorel_logger.debug(
            f"Scoring finished on process {self.accelerator.process_index}"
        )
