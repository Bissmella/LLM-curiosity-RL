...
class HF_LLM(BaseLLM):
        ...
        for idx, output in enumerate(outputs):
            ...
            if inputs_representation is None:
                batch["input_ids"].append(
                    inputs[idx]["input_ids"] + _output["input_ids"]
                )
            else:
                batch["input_ids"].append(_output["input_ids"])

            batch["attention_mask"].append(
                inputs[idx]["attention_mask"] + _output["attention_mask"]
            )
    ...
    def generate(self, contexts,prompts=["<MORE_DETAILED_CAPTION>"],sample=False, return_logprobs=False,temperature=1, **kwargs):
        generations = []
    ...
    def forward(
        ...
        **kwargs,
        ...
        with torch.no_grad() if not require_grad else nullcontext():
            ...
            tokenized_contexts = [
                self._LLM_tokenizer(
                    context, return_token_type_ids=False, add_special_tokens=False
                )
            ...
            for _w, _candidates in enumerate(candidates):
                ...
                outputs = [
                    self._LLM_tokenizer(
                        output, add_special_tokens=False, return_token_type_ids=False
                    )
                    for output in _candidates
                ]
                if self.model_type == "causal":
                    cut_input = {
                        _k: _v[:-1] for _k, _v in tokenized_contexts[_w].items()
                    }
