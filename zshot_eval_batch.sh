...
for ((i=0; i<NUM_TASKS; i++)); do
    ...
    for MODEL in "${MODELS[@]}"; do
        for LORA in "${USE_LORA[@]}"; do
            ...
            JSON_PATH="/home/bahaduri/VIPER/outputs/full_eval_${TASK}_LORA_${LORA}_model_${MODEL}.json"
            ...
            bash eval_zshot.sh  "$TASK" "$LORA" "$MODEL_PATH" "$LOG_PATH" "$RESULTS_FILE" "$TASK_NAME" "$JSON_PATH"
        done
    done
done